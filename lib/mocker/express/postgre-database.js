...
postgre.getQuery = function(req) {
	...
	let ast_id = postgre.getAstId(api_id, req.method, req.query, req.body, pathname);
...
//TODO DB에서 api로 필수메서드 및 {}형태의 path를 조회하도록 수정해야 함.
postgre.getAstId = function(api_id, method, query, body, pathname){
	let ast_id;
	...
	switch(api_id) {
		...
		case '509':
		case '510':
			ast_id = postgre.getPathVal(pathname, 2);
			break;
	}
	return ast_id;
};
