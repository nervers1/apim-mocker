				...
				(async () => {
					...
					try {
						...
						if(limit){
							...
							if(data.trans_cnt){
								let trans_cnt = data.trans_cnt;
								data.trans_list = this.paginate(data.trans_list, limit, page);
								let next_page = this.getNextPage(trans_cnt, page, limit);
								if(next_page) data.next_page = next_page;
							}
							else if(data.bill_detail_cnt){
								let bill_detail_cnt = data.bill_detail_cnt;
								data.bill_detail_list = this.paginate(data.bill_detail_list, limit, page);
								let next_page = this.getNextPage(bill_detail_cnt, page, limit);
								if(next_page) data.next_page = next_page;
							}
							else if(data.account_cnt){
								let account_cnt = data.account_cnt;
								data.account_list = this.paginate(data.account_list, limit, page);
								let next_page = this.getNextPage(account_cnt, page, limit);
								if(next_page) data.next_page = next_page;
							}
						}
