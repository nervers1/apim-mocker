						...
						if(limit){							
							...
							if(data.trans_cnt){
								...
								data.trans_list = data.trans_list.filter(function (a) {
									var hitDates;
									if(a.trans_dtime && a.trans_dtime.length > 8)
										hitDates = new Date(a.trans_dtime.replace(
											/^(\d{4})(\d\d)(\d\d)(\d\d)(\d\d)(\d\d)$/,
											'$4:$5:$6 $2/$3/$1'
										));
									else if(a.trans_date)
										hitDates = parent.parseStringYYYYMMDD(a.trans_date);
									else if(a.trans_dtime)
								...
								data.bill_detail_list.sort(function(a,b) {
									return parseFloat(b.bill_dtime) - parseFloat(a.bill_dtime);
								});
								//2. date 검색
								data.bill_detail_list = data.bill_detail_list.filter(function (a) {
									var hitDates = new Date(a.paid_dtime.replace(
										/^(\d{4})(\d\d)(\d\d)(\d\d)(\d\d)(\d\d)$/,
										'$4:$5:$6 $2/$3/$1'
									));
									return hitDates >= startDate && hitDates <= endDate
